...
function extractresonancefreq(singlet_0ppm::SingletFIDParameters{T})::T where T <: AbstractFloat
    ν_0ppm = singlet_0ppm.Ω / twopi(T)
    return ν_0ppm
end

function extractfreqrefHz(A::OutputContainer1D{T,ST})::T where {T <: AbstractFloat, ST}
    return extractresonancefreq(A.freq_ref)
end

function extractsolventfHz(A::OutputContainer1D{T,ST})::T where {T <: AbstractFloat, ST}
    return extractresonancefreq(A.solvent)
end

function extractfreqreferror(A::OutputContainer1D{T,ST})::T where {T <: AbstractFloat, ST}
    return A.freq_ref.relative_error
end

function extractsolventferror(A::OutputContainer1D{T,ST})::T where {T <: AbstractFloat, ST}
...
function FrequencyConversionParameters(A::OutputContainer1D{T,ST})::FrequencyConversionParameters{T} where {T <: AbstractFloat, ST}
...
function hz2ppm(u_Hz::T, A::FrequencyConversionParameters{T})::T where T
    return (u_Hz - A.ν_0ppm)*A.SW/A.fs
end

function ppm2hz(p_ppm::T, A::FrequencyConversionParameters{T})::T where T
