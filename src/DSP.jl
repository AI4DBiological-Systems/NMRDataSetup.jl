...
"""
function gettimerange(N::Int, fs::T)::LinRange{T, Int} where T
    Ts::T = convert(T, 1/fs)
...
"""
    computeDTFT(h, u::T, X)::Complex{T} where T <: Real
...
"""
function computeDTFT(h, u::T, X)::Complex{T} where T <: Real
...
"""
getinversemap(forward_mapping::Vector{Int})::Vector{Int}
...
```
"""
function getinversemap(forward_mapping::Vector{Int})::Vector{Int}
