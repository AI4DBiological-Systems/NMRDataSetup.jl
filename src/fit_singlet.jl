...
function evalFID(t::T, α::T, β::T, λ::T, Ω::T)::Complex{T} where T <: Real
    return α*cis(Ω*t + β)*exp(-λ*t)
end

function evalCL(u_rad::T, α::T, β::T, λ::T, Ω::T)::Complex{T} where T <: Real
...
function evalCLsplit(u_rad::T, a::T2, b::T2, c::T2, d::T2)::Tuple{T2,T2} where {T <: Real, T2 <: Real}
...
function evalAL(u, α, β::T, λ, Ω)::Complex{T} where T <: Real
...
function evalDL(u, α, β::T, λ, Ω)::Complex{T} where T <: Real
